...
const ENABLED_TYPES = TYPE_MAP.filter(function(m) { return $config[m.front] !== false; }).map(function(m) { return m.back; });
const B2F = {};
TYPE_MAP.forEach(function(m) { B2F[m.back] = m.front; });
...
async function getCards(ext) {
    ...
    try {
        ...
        const prio = {};
        ($config.pan_priority || []).forEach(function(k, i) { prio[k] = i; });

        const list = [];
        Object.keys(data).forEach(function(bKey) {
            const items = data[bKey];
