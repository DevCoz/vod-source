...
const B2F = {}
Object.keys(PAN).forEach(k => { B2F[PAN[k]] = k })
...
async function getCards(ext) {
    ...
    try {
        ...
        const prio     = ($config.pan_priority || []).reduce((o, k, i) => ({ ...o, [k]: i }), {})

        const list = Object.entries(data).flatMap(([bKey, items]) =>
            (items || []).map(item => ({
                vod_id:      item.url,
